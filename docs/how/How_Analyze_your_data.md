...
**Preparing the example** — If you'd like to play along, [add these libraries](How:Add_a_Library.md) and [fields](../ui/Library_Edit_screen.md). Alternatively, you could just read along and apply analogous actions to your own library:
- **Customers** with unique entry name and the following fields:
- **Name** &mdash; single-line [Text field](../fields/Text_field.md), entry name
- **Type** &mdash; [Radio Buttons field](../fields/Radio_Buttons_field.md), entry status, with items **Consumer** and **Institution**
- **State** &mdash; [Radio Buttons field](../fields/Radio_Buttons_field.md), entry status, with items as you like (for instance, **NY**, **OR**, and **TX**)
- **Products** with unique entry name and the following fields:
- **Name** &mdash; single-line [Text field](../fields/Text_field.md), entry name
- **Category** &mdash; [Radio Buttons field](../fields/Radio_Buttons_field.md), entry status, with items **Lumber** and **Tools**
- **Cost** &mdash; [Currency field](../fields/Currency_field.md), entry status
- **Std price** &mdash; [Currency field](../fields/Currency_field.md), entry status
- **Orders** with unique entry name and the following fields:
- **Customer** &mdash; one-to-many [Link To Entry field](../fields/Link_To_Entry_field.md) to library **Customers**, entry name
- **Date** &mdash; [Date field](../fields/Date_field.md), entry name
- **Quantity** &mdash; [Integer field](../fields/Integer_field.md), entry description
- **Product** &mdash; one-to-many [Link To Entry field](../fields/Link_To_Entry_field.md) to library **Products**, entry description
- **Price** &mdash; [Currency field](../fields/Currency_field.md), entry description
...
**Add these fields to Orders** — :* **Customer type** &mdash; [Calculation field](../fields/Calculation_field.md), string result, entry status
- Press the **+Field** button, choose **customer.type** from the list
- **Customer state** &mdash; [Calculation field](../fields/Calculation_field.md), string result, entry status
- Press the **+Field** button, choose **customer.state** from the list
- **Product category** &mdash; [Calculation field](../fields/Calculation_field.md), string result, entry status
- Press the **+Field** button, choose **product.category** from the list
- **Product cost** &mdash; [Calculation field](../fields/Calculation_field.md), real result, scale 2, entry status
- Press the **+Field** button, choose **product.cost** from the list
- **Product std price** &mdash; [Calculation field](../fields/Calculation_field.md), real result, scale 2, entry status
...
**Add these fields to Orders** — :* **Product margin** &mdash; [Calculation field](../fields/Calculation_field.md)
...
```
- **Product margin %** &mdash; [Calculation field](../fields/Calculation_field.md)
Enter this script:
```
...
```
- **Order margin** &mdash; [Calculation field](../fields/Calculation_field.md)
Enter this script:
```
...
### Aggregation
There is nothing to aggregate in **Customers**. In the **Products** **[Library Edit screen](../ui/Library_Edit_screen.md)**, under the **Aggregation** tab, add the following aggregations;
- Sum **Std price** on the left
- Sum **Cost** on the right on the same line

In the **Orders** **[Library Edit screen](../ui/Library_Edit_screen.md)**, under the **Aggregation** tab, add the following aggregations:
...
In this case, let's define filters for each product category. On the **[Library Entries List screen](../ui/Library_Entries_List_screen.md)**, define the following filters:
...
**Ad hoc filtering** — Choose from the list of filters you've previously defined to focus in on the data you want to analyze, choose no filter to analyze all the data, or just filter on the fly.
 With [filter tabs](../ui/Library_Entries_List_screen.md#Filter_Tabs) (available in mobile editions starting with release 4.0), you can employ filters in an enhanced manner.
