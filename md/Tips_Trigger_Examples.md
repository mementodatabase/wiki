...
---
      1. Previous value of another field
...
---
      ...
      1. Beginning of the next day
Suppose you need to identify the beginning of a new day in the DateTime field. (The script requires connection of the JavaScript library **moment.js** [moment.js](https://momentjs.com/).)
...
---
      ...
      1. New entry in one library triggers new entry in another
...
**Fields** — Library Applications has fields **Date**, **Name**, City, Years in city, Type.
 Library Members has fields **Name** & Type.
**Set library permission** — The Applications library must be permitted to access other libraries via *libByName()*.
Within Triggers, press the shield icon to view Permission settings.
**Add new trigger** — Add a new trigger to the Applications library.
...
---
      ...
      1. Ensuring unique non-Name field value
...
**Add new trigger** — Add a new trigger to the library.
Set Event **Creating a new entry**, Phase **Before saving the entry**.
(A similar trigger could be set for Event **Updating an entry** Phase **Before saving the entry** &mdash; for instance, if the library already has entries in which *myField* may not be unique.)
...
---
      ...
      1. TEMPORARY SECTION. PLEASE DISREGARD
...
**Libraries** — My Today's Activities &mdash; Contains a member's plan for the day.
Activity Plans &mdash; Essentially a menu of available plans for activities.
**Fields** — My Today's Activities &mdash; Date and Member, a few other fields, and a Checkboxes field called **Today's Plan**.
Activity Plans &mdash; A plan name, description, and a Checkboxes field **Plan** (identical items to Today's Plan) with the checked activities associated with the plan.
**Add new trigger** — Add two new triggers to the library.
Set Event **Creating a new entry**, Phase **Before saving the entry**.
...
```
**After the Creating/Before trigger is running correctly** — : Add another trigger, this time for event **Updating an existing entry** and phase **Before saving the entry**. Then copy the previous script to the clipboard and paste it into this new trigger, so that they are identical, except for the trigger name and the event name.

---

      1. Copying the value of a Checkboxes or Multiple-choice field to another library
...
---
      ...
      1. Copying an entry's field values to another library
...
---
      1. Save an entry to a file in XML format
...
var xml = '' +  // Format XML record
'' + entry().field("title") + '' +        //     from entry field values
'' + entry().field("date") + '' +
'';
...
---
      ...
      1. Creating a Task in the Todoist App
[Todoist](https://todoist.com) — A Web service and mobile app for task management. Todoist allows task creation via API [In the following example of task creation, text will be taken from the Memento library entry.
...
//     using the format described in [https://develop.todoist.com/#add-an-item](https://developer.todoist.com/].).
...
---
   ...
   1. System Examples
---
      1. Script to open a screen for dialing a number
...
---
      1. Script to open app to send SMS message
...
---
      1. Script to insert an appointment into Google Calendar
...
---
      1. Example: Extracting components from a Contact field into separate fields
...
**CreateBefore** — : Event: Creating a new entry
...
**UpdateBefore** — : Event: Updating an existing entry
...
Now, every time you create a new entry or update an existing one, the contact information will be extracted to separate fields.
      ...
      1. Example: Extracting coordinates from a Location field into Real fields
...
---
   ...
   1. See Also
**[Triggers](Triggers.md)** — Fundamental information about Memento triggers
...
[Catgegory:en](Catgegory:en.md)  [Categgory:Tips](Categgory:Tips.md)
